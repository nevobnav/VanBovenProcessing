...
def MetashapeProcess(photoList, day_of_recording, metashape_processing_folder, ortho_out, quality, scan_time):
    ...
    try:
        ...
        print('no console attribute available')
    ...
    tic = time.clock()
    chunk.matchPhotos(downscale=2, generic_preselection=False, reference_preselection=True, filter_mask=False, keypoint_limit=40000, tiepoint_limit=4000)
    ...
    toc = time.clock()
    processing_time = toc - tic
    logging.info("DepthMap generation took " + str(int(processing_time)) + " seconds")
    tic = time.clock()
    chunk.buildDenseCloud(max_neighbors = 100, point_colors = True)
    toc = time.clock()
    processing_time = toc - tic
    ...
    tic = time.clock()
    chunk.buildDem(source_data = Metashape.DenseCloudData, interpolation=Metashape.EnabledInterpolation, projection=proj)
    toc = time.clock()
    processing_time = toc - tic
    ...
    # - Blending mode in [AverageBlending, MosaicBlending, MinBlending, MaxBlending, DisabledBlending]
    tic = time.clock()
    chunk.buildOrthomosaic(surface_data=Metashape.ElevationData, blending_mode=Metashape.MosaicBlending, projection=proj)
    toc = time.clock()
    processing_time = toc - tic
    ...
    chunk.exportRaster(path = ortho_out, source_data=Metashape.OrthomosaicData, image_compression = compression)
    #chunk.exportOrthomosaic(path = ortho_out, tiff_big = True, tiff_tiled = True) #, jpeg_quality(75)
    chunk.exportRaster(path = DEM_out, source_data=Metashape.ElevationData, image_compression = compression)
...
logging.basicConfig(filename = os.path.join(root_processing_path, "Log_files/" + str(timestr) + "_Metashape_log_file.log"),level=logging.DEBUG)
